...
public class CombinationsOfCoins {
    ...
    private void helper(int target, int[] coins, int index, List<Integer> cur, List<List<Integer>> result) {
        // terminate condition:
        // NOTICE： this can also be done at index == coin.length - 1 where all the coins have been picked.
        // but a probably better one is at a previous level to reduce the number of
        // unnecessary branches in DFS
        // think about it, coins.length - 1 represent the last coin we can use and actually
        // what we can do at this level is to get target / coins[length - 1] coins if possible
